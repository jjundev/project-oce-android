 ...
 */
@Suppress("VariableNaming", "ConstNaming")
object OverscrollDefaults {
    ...
    const val SNAP_TO_HOLD_MS: Int = 140
    const val MIN_VISIBLE_MS: Long = 450L
    const val WAVE_DURATION_MS: Float = 520f
    const val WAVE_STAGGER_MS: Float = 36f
    val WaveCardPeak: Dp = 11.dp
    val WaveHeaderPeak: Dp = 4.5.dp
    const val BURST_COUNT: Int = 13
    const val BURST_FLY_MS: Int = 680
    val BurstMinDist: Dp = 42.dp
    val BurstMaxDist: Dp = 80.dp
    val BurstMinSize: Dp = 5.dp
    val BurstMaxSize: Dp = 10.dp
    const val SPRING_DAMPING_RATIO: Float = 0.32f
    const val SPRING_STIFFNESS: Float = 220f
}
...
fun rubberBand(
    rawDragPx: Float,
    maxPx: Float,
): Float = if (rawDragPx <= 0f) 0f else maxPx * (1f - 1f / (1f + rawDragPx / maxPx))

/** [rubberBand] 의 역함수 — 현재 오프셋에서 누적 드래그를 복원(드래그 재개 시 연속성 유지). */
fun inverseRubberBand(
    offsetPx: Float,
    maxPx: Float,
): Float = if (offsetPx <= 0f) 0f else offsetPx / (1f - offsetPx / maxPx)
