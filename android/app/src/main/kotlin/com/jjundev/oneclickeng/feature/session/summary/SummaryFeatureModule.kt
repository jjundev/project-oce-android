 ...
 * 은 core `SummaryModule` 이 바인딩하고, 여기서는 요약 화면 고유의 두 seam 을 바인딩한다:
 * - [BookmarkSource] → [EmptyBookmarkSource] (M2-04 착지 전 빈 리스트; M2-04 가 Firestore 구현으로 교체).
 * - [CompletionLedger] → [FirestoreCompletionLedger] (요약 진입 시 point_ledger create 시도, #20).
 *
 * 코디네이터가 쓰는 앱 CoroutineScope 는 `TtsProvideModule.provideAppScope` 가 제공한다(M1-05 공유).
 */
@Module
...
abstract class SummaryFeatureModule {
    @Binds
    @Singleton
    abstract fun bindBookmarkSource(impl: EmptyBookmarkSource): BookmarkSource
