...
class ReviewViewModelTest {
    ...
    @Test
    fun `loads pool and starts on first card with type-specific phase`() =
        runTest(dispatcher) {
    ...
    @Test
    fun `quiz correct pick records srs box 1 and reveals`() =
        runTest(dispatcher) {
    ...
    @Test
    fun `flashcard reveal then done advances and finishes at end`() =
        runTest(dispatcher) {
    ...
    @Test
    fun `quiz wrong pick counts again and records box 1`() =
        runTest(dispatcher) {
            val (viewModel, repo) = vm(listOf(expr))
            advanceUntilIdle()
            viewModel.pick(0)
    ...
    @Test
    fun `ahead-of-schedule pool flags uiState and grading skips srs update`() =
        runTest(dispatcher) {
    ...
    @Test
    fun `non-ahead pool does not flag aheadOfSchedule`() =
        runTest(dispatcher) {
