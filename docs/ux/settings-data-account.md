...
### 2.1 실현 반영 (2026-07-07, `Prototype Flow` 정합 · 비준 대기)

Compose 구현을 `prototype/Prototype Flow` 설정 화면에 맞추며 아래를 반영했다. 시각(카드 그룹·아이콘)은 realization-SoT를 따르고, **아래 마이크로카피·프로필 인터랙션은 본 문서(ux 정본)가 소유**하므로 여기 기록해 코드↔문서 정합을 맞춘다(값은 프로토 렌더에서 이식, 최종 문안은 비준 대상).

- **섹션 카드화**: 각 섹션을 `surface.card` + hairline 그룹으로 묶고 행 사이 hairline 구분선([04-screen-07-settings.md](../ui/04-screen-07-settings.md) ST1 rev2 정합).
- **행 선행 아이콘**(Material Symbols): 프로필=`person`, 음질=`graphic_eq`, 말하기 속도=`speed`, 전체 음소거=`volume_up`, 학습 리마인더=`notifications`.
- **행 보조 마이크로카피**(이식): 말하기 속도="TTS 재생 속도예요.", 전체 음소거="텍스트만 보고 자동으로 넘어가요.", 학습 리마인더="매일 지정 시각에 알려드려요.".
- **음질 순서·보조 문구**(⚠️ 스펙 §2·#3 순서와 상충, 비준 필요): 세그먼트 순서를 프로토 정합 `[빠른 발음 | 자연스러운 발음]`로 변경(§2 와이어프레임은 자연스러운 먼저). 보조 문구는 **선택된 옵션 설명(동적)** — 빠른 발음="단말 합성 · 즉시 재생돼요."(이식), 자연스러운 발음="서버 합성 · 더 자연스러워요."(**초안**). 정적 device 문구가 server 기본과 모순되던 문제 해소. server 문구·순서 뒤집기는 제품/UX 비준 대상.
- **말하기 속도**: 슬라이더 하단 눈금 라벨 `0.5x·1.0x·1.5x` + 우측 현재값(예 `1.0x`).
- **프로필 닉네임**: 위 §2 와이어프레임의 인라인 `[______]` → **표시행(값) + "변경하기" 버튼 → 편집 다이얼로그**(1~20자·빈 값 허용, 결정 #8 유지)로 실현. behavior 변경이므로 비준 대상.
