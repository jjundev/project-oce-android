...
**모델 교체(2026-09-23):** dialogue/speaking/summary는 `gemini-3.5-flash-lite`(thinkingLevel `MINIMAL`)로 옮겼고, feedback/feedbackDeep는 `gemini-3.1-flash-lite`에 **남겼다**. 같은 골든셋(14케이스 × 3회, t=0)에서 3.5는 학습자 오류 미표시·과잉 교정이 5건(MINIMAL)·7건(LOW)이었고(3.1은 0건), 점수 표준편차도 평균 1.6~2.0(3.1은 0.0)이었다. deep은 두 모델이 비슷했다(말투 위반 8 vs 10). 3.1은 2027-05-07에 종료되므로 그 전에 피드백 프롬프트를 3.5에 맞게 다시 조정하고 eval로 재확인해야 한다. `functions/src/config/models.ts`가 SoT다.
...
**확정(2026-07-18, 프롬프트 문안):** feedback 프롬프트에 (a) 금지 어미를 이름으로 지목한 해요체 규칙(`-입니다`/`-습니다`/`-ㅂ니다` 금지, `-답니다`/`-랍니다`는 허용, 칭찬 문맥을 명시 경고), (b) 5레벨 LEVEL 블록(`explanation`/`reason.description`과 제안 문장만 레벨에 따라 적응시키고 `writingScore.score`는 절대 유지) (c) `korean-error-reference`의 8개 오류 계열을 COMMON ERRORS로 접어 넣었다. `functions/eval/`로 동일 명령(`--temps=0 --repeats=5 --task=feedback`, 70콜)을 수정 전후로 재서 측정: 말투 위반 40건 → 0건, `compareLevelSensitivity` 판정 실패(비교된 5회 중 설명 동일 0회·제안 문장 동일 5회 — 설명은 레벨을 언급하기 전에도 매 회 달랐고, 제안 문장만 레벨을 무시했다) → 통과(비교된 5회 모두 설명·제안 문장이 레벨에 따라 달라짐), 구조 위반은 전후 모두 0건 유지. **이 통과가 증명하는 것:** `naturalExpression` 쪽 판정은 프롬프트가 명시한 "같은 영어가 starter/expert에서 같은 제안 문장으로 돌아오면 안 된다"는 지시와 사실상 같은 문장을 검사하므로, 통과는 그 **지시가 이행되고 있다는 것**을 증명할 뿐 제안된 난이도가 학습자에게 실제로 적절한지는 증명하지 않는다 — 그 판단은 여전히 사람이 리포트의 렌더링된 출력을 읽고 내려야 한다. "레벨 민감도가 확인됐다"고 읽으면 과장이다. 리포트: `functions/eval/out/feedback-2026-07-18T12-56-04-939Z.md`(기준선), `functions/eval/out/feedback-2026-07-18T13-23-07-699Z.md`(수정 후). `FEEDBACK_PROMPT_VERSION = "2026-07-18"`.

부수 효과로 점수가 14케이스 중 8케이스에서 5~10점 하향 이동했고(상향은 0건), **가장 유력한 원인으로 보이는 것은** COMMON ERRORS 블록이 새로 접히면서 모델이 콩글리시·관사·전치사 같은 오류 계열을 이전엔 넘어가던 사례에서도 잡아내 더 엄격하게 채점한 것이다. 다만 이는 **입증되지 않았다** — 같은 커밋 묶음에서 함께 나간 (b)의 좁혀진 "already natural" 예외와 *"go BEYOND `referenceEnglish`"* 지시도 대등하게 유력한 후보다: 제출한 영어에는 거의 항상 더 나은 표현이 있다고 모델에게 알려주는 지시는, 0-100 품질 판단을 같은 방향으로(같은 케이스에서, 하나도 오르지 않고) 낮추는 효과를 낼 수 있고, 이는 관측된 신호(8/14 하향, 상향 0)와 정확히 같은 모양이다. 두 변경이 같은 스윕에서 함께 나갔기 때문에 이 컨파운드를 가르는 측정은 없다 — COMMON ERRORS 블록만 켜고 (b)의 좁혀진 예외 조항은 되돌린 70콜 ablation 스윕(`--temps=0 --repeats=5 --task=feedback`)이면 가려낼 수 있다. 원인 규명과 무관하게, 사용자에게 보이고 채점 밴드를 재조정하지 않은 채 수용하기로 한 것은 의도적 결정이다 — "hand phone"이 95점을 받던 것이 과했다는 판단이며, 아직 사용자가 없어 기준선을 옮기기 가장 저렴한 시점이라는 점도 근거로 들었다. 숫자와 수용 결정 자체는 바뀌지 않는다.
