...
## 3. 합성 · 전달
- **라인당 합성**(전체 대본 일괄 아님). Gemini TTS → base64 PCM(24kHz) → **일반 JSON HTTPS 응답**(텍스트 SSE 아님) → 클라 디코드 → AudioTrack.
...
- 프록시-TTS · 단말 폴백은 **신규**(옛 앱은 Gemini 직접 호출, 폴백 없음). 프록시는 응답 2모드(텍스트 SSE / TTS PCM JSON)를 갖는다.
