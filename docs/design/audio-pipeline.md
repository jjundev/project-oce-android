...
**범위 밖(명시):** **슬림 문장 피드백(작문 점수·문법·표현, 스트리밍) 호출은 B-3가 아님** — 대화 feature + B-1(프롬프트) 소관. B-3는 `{transcript, encouragement}`까지 만들고 transcript를 feature 레이어로 핸드오프한다. (옛 앱도 `ISpeakingFeedbackManager`[오디오·단항]와 `ISentenceFeedbackManager`[텍스트·스트리밍]가 분리돼 있었음.)
...
- **`MicState.Complete`는 `/speaking` 반환 시점**에 발동한다 — 범위 밖 문장 피드백 호출 완료와 **무관**(안 그러면 Complete를 영영 못 기다림).
...
feature/dialogue/audio/
  SpeakingController.kt  (record→silence→/speaking→MicState 오케스트레이션)
ui/audio/
...
interface SpeakingAnalysisClient {               // 백엔드 프록시
  suspend fun analyze(sessionId:String, pcm:ByteArray, sampleRate:Int): SpeakingResult          // {transcript, encouragement}
}
```
...
- **TTS fetch:** `TtsClient.synthesize(text, voice, locale, speechRate)` → 프록시 `POST /tts` → `{pcmBase64, sampleRate}` — **일반 JSON 응답**(텍스트 SSE 아님), base64 PCM 24kHz ~190-380KB/라인. `voice`=생성기 `opponent_gender` 매핑(male=`Puck`/else=`Kore`), `locale`=en-US, `speechRate`(0.5~1.5, 백엔드 프롬프트 베이크). 상세 [tts.md](tts.md) §1-3.
...
재탭/20s → stop():
  ...
  Captured → MicState=Analyzing → SpeakingAnalysisClient.analyze(pcm)
           → {transcript, encouragement} → MicState=Complete
...
## 9. 백엔드 계약 (B-2 의존 — 비준 필요)
> `/speaking`·`/tts`는 현재 미존재. B-3가 **B-2(LLM 프록시)에 요구하는 제안 계약**이며, **B-2 설계 시 비준** 전까지 end-to-end 검증 불가.

- `POST /speaking` `{sessionId, audioBase64, sampleRate}` → `{transcript, encouragement}` (백엔드가 Gemini inlineData 래핑·키 보관). 업로드 = raw PCM16 base64(클라가 WAV 안 감쌈). 20s ≈ 853KB(≪10MiB 콜러블 캡), 업로드 ~0.5-1.5s를 **NFR-3 턴 예산에 계상**. (압축 Opus는 캡 상향 시.)
- `POST /tts` `{text, voice, locale, speechRate}` → `{audioBase64, sampleRate}`.
...
| 업로드 | raw PCM16 base64 → /speaking | 벤더 포맷 백엔드 소유 |
| 캡 | 20s | NFR-3 예산 정합 |
...
| 범위 | transcript+encouragement까지(문장 피드백 제외) | 옛 2 feedback manager 이음새 |
...
**의존성:**
- **B-2(LLM 프록시):** `/speaking`·`/tts` 계약 비준 필요(§9). 그 전 end-to-end 검증 불가.
- **B-1(프롬프트):** transcript를 소비하는 슬림 문장 피드백 호출(범위 밖).

**v1 한계:** 무음 게이트 임계(0.02/0.1/0.05)·gain(3.0)·캡(20s)·워치독(7s)은 단말 편차로 운영 보정. auto-VAD(말 끝나면 자동 정지)는 후속. raw PCM(무압축) — 대역폭 이슈 시 Opus.

## 13. 검토 이력
`grill-review --deep auto` (2 iteration):
- **iter1:** Blocker 5(VoiceState 4상태가 상대역 재생 못 담음 · FR-10 2번째 호출 미명세 · TTS 워치독 누락 · 60s base64 vs NFR-3 · /speaking·/tts 미존재 의존) + Advisory 9 → 수정.
